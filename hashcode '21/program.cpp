...
int main()
{
	...
	cin.tie(NULL);
	cout.tie(NULL);
	freopen("f.txt","r",stdin);freopen("f_out.txt","w",stdout);
	...
	unordered_map<int, vector<pii>> ans;
	for (int j = 0; j < i; ++j)
	{
	...
	for (auto &vec : ans)
	{
		...
		for (auto &v : vec.S)
			cout << streetsInv[v.S] << ' ' << max((int)1, (int)(((long double)ans[v.F].size()*(long double)2) / sum)) << '\n';
	}
